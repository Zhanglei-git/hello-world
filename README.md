...
2. 将输入historyData按全局Id存入数组recodsArray，数组每个元素为一个Id所对应的快照

3. 对recodsArray进行遍历,逐个获取每条记录的所有动物快照存入map
...
4. 获取单条记录的动物快照时，先验证该条记录格式的正确性，（行数、id格式、时间格式等）
   3.1 若格式错误，则设置errMsg并返回false，结束遍历；
   3.2 若格式正确，则取出该记录中动物的位置信息加入map(遍历取出所有)
       加入map时：验证动物位置信息格式（放在此处验证而没有在上面一块验证，是为了减少遍历次数）
       3.2.1 若动物位置信息格式正确：
             3.2.1.1 若map中已有该动物，比较与已有动物快照是否相对应，是则更新，否则设置errMsg，并返回false。
	     3.2.1.2 若map中没有该动物，则直接加入。
       3.2.2 若动物位置信息格式错误:  
             设置errMsg并返回false，结束遍历；

当步骤2遍历结束时，map中即为Id时刻的所有动物快照。
